...
Jumper.Play.prototype = {
	...
	preload: function() {
		this.load.spritesheet( 'hero', './img/den.png', 50, 74, 4);
		this.load.image( 'pixel', './img/pixel_1.png' );
	},
	...
	create: function() {
		...
		this.counter = game.add.text(game.world.width-10, 20, "0", { font: "20px Arial", fill: "#fff", align: "rigth" });
		this.counter.fixedToCamera = true;
		this.counter.anchor.set(1, 0.5);
	...
	update: function() {
		game.physics.arcade.overlap(this.hero, this.platforms, this.collisionHandler, null, this);
		...
		if (this.isFly){
			this.hero.frame = game.rnd.integerInRange(1, 3);
		} else {
			this.hero.frame = 0;
		}
		...
		this.platforms.forEachAlive( function( elem ) {
			...
			if( elem.y > this.camera.y + this.game.height ) {
				elem.kill();
				this.platformsCreateOne( this.rnd.integerInRange( 0, this.world.width - 50 ), this.platformYMin - 70, 50 );
			}
		}, this );

		this.counter.text = Math.abs(parseInt(this.camera.y));
	},
	...
	platformsCreate: function() {
		...
		for( var i = 0; i < 9; i++ ) {
			this.platformsCreateOne( this.rnd.integerInRange( 0, this.world.width - 50 ), this.world.height - 70 - 70 * i, 50 );
		}
	},
	...
	heroCreate: function() {
		// basic hero setup
		this.hero = game.add.sprite( this.world.centerX, this.world.height - 74, 'hero' );
